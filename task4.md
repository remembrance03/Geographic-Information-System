...
6. Click on the map sheet index polygon that covers your municipality.
7. Note down the **sheet box / sheet code** (e.g., B A B D C D B A B A C D).
8. Confirm that the identified sheet fully covers Panchkhal Municipality.
...
5. If the file is compressed, extract it.
6. Store the extracted image in your project folder **outside the file geodatabase (.gdb)**.
...
3. Set the coordinate system to the same system used by your vector data  
   (e.g., *Nepal National Grid* or the projection used for Kavrepalanchowk data).
4. Click **OK**.
...
2. Turn the raster layer on and off to visually inspect alignment.
3. If the image flips or shifts:
   - Delete incorrect control points
   - Re-add them carefully
4. Ensure roads, rivers, and boundaries align reasonably well.
